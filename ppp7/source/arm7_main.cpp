...
//#define REG_DEBUG *((u32*)0x4042000) // tweak your emulator to support this.
...
extern bool sleepIsEnabled; // from libnds/arm7/system.c
...
//---------------------------------------------------------------------------------
    ...
    initClockIRQ(); // includes rtcReset and more.
#ifdef WIFI
    installWifiFIFO();
#endif
    installSystemFIFO();
    ...
    // Keep the ARM7 out of main RAM, mostly idle.
    while (1) {
      swiWaitForVBlank();
    }
}
