	...
	if(!strcmp(ext2, ".nds") || !strcmp(ext2, ".bin") || !strcmp(ext, "ds.gba")) {
		if((f = open(path, O_RDWR))) {
			...
			if(size == efs_filesize) {
				...
				iprintf("scanning for magic string %02x%02x%02x%02x%s ...\n",
					magicString[0],magicString[1], magicString[2], magicString[3],
					magicString+4);
				...
				while(efs_offset<size && !found) {
				  ...
				  for(i=0; i < dataChunk_size; i++)   {
				      if (efs_offset+i >= 0x7a0 && efs_offset+i<0x7B0)
					iprintf("candidate @%x[%02x%02x%02x%02x%s] vs %x\n", efs_offset, 
						dataChunk[i],dataChunk[i+1], dataChunk[i+2], dataChunk[i+3],
						dataChunk+i+4, efs_id);
				      if(dataChunk[i] == magicString[0]) {
					  if(dataChunk_size-i < 12)    {
					      iprintf("chunk too short : %i bytes left\n",dataChunk_size -i);
					      break;
					    }
					  if(memcmp(&dataChunk[i], magicString, 12) == 0) {
					    ...
					    iprintf("chunk mismatch @%x: \n%.30s\n",efs_offset, dataChunk);
					}
				    }
				  if(!found)    {
				...
				// check file id
				int testme=(dataChunk[i+12] | (dataChunk[i+13]<<8) | (dataChunk[i+14]<<16) | (dataChunk[i+15]<<24));
				iprintf("checking efs_id %x\n",testme);
				// dataChunk[i+12], dataChunk[i+13], dataChunk[i+14], dataChunk[i+15]);
				if(found == true && efs_id == testme)  {
				   //(*(int*)(dataChunk+i+12) == efs_id))
				  iprintf("EFS id matching %x!\n",efs_id);
				  strcpy(efs_path, path);
				  // store file path in NDS
				  if(save) {
				    iprintf("saving the path\n");
...
bool SearchDirectory()
{
	...
	dir = opendir(".");
	// iprintf("@%p",dir);
	while(( (dent = readdir(dir)) != 0) && (!found)) 	{
	  if(dent->d_type == DT_DIR) {
	    if(((strlen(dent->d_name) == 1) && (dent->d_name[0]!= '.')) ||
	       ((strlen(dent->d_name) == 2) && (strcasecmp(dent->d_name, "..")))  ||
	       (strlen(dent->d_name) > 2)) {
	      chdir(dent->d_name);
	      iprintf(" %s [",dent->d_name);
...
// look if the linker is inside the gba slot and the type of rom
bool EFS_InitGbaSlot(bool* found)
{
...
// initilize the efs's fat system if is need
bool EFS_InitFat(bool* found, int options, char *path)
{
	...
	iprintf("EFS: check if the provided path is valid\n");
	if(path && CheckFile(path, true)) 	{
		*found = true;
	} 	else 	{
	  iprintf("EFS: check if there's already a path stored\n");
	  if(efs_path[0]) 		{
	  ...
	  iprintf("EFS search the whole FAT (press A to proceed)\n");
	  iprintf("(ARGV-compatible linkers do not need this)\n");
	  while(!keysHeld()) scanKeys();
	...
	if(*found) 	{
	  ...
	  iprintf("all tested and none found.\n");
	}
...
int EFS_Init(int options, char *path)
{
	...
	if(options & EFS_FAT_PRIO) 	{
		if(!EFS_InitFat(&found, options, path))
			EFS_InitGbaSlot(&found);
	} 	else 	{
		if(!EFS_InitGbaSlot(&found))
			EFS_InitFat(&found, options, path);
	}
	...
	iprintf("EFS found: %c, useDLDI: %c, ndsfile: %i\n",(found?'t':'f'),
		useDLDI?'t':'f',nds_file);
