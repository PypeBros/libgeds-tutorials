...
void* _malloc(size_t size) {
  if (size==0) {
    printf("!! SOMEONE REQUESTS 0 BYTES !!\n");
  ...
  if (p) {
    ...
    printf("!! OUT OF MEMORY (less than %i bytes)!!\n",size);
    return 0;
  }
}
...
void* _calloc(size_t nmemb, size_t size) {
  ...
  void *p = malloc(nmemb*size+4);
  if (p) {
    memset(p, 0, nmemb*size+4);
    memcpy(p, "pYxM",4);
    return (void*)(((char*)p)+4);
  } else {
    printf("!! OUT OF MEMORY (less than %i bytes)!!\n",nmemb*size);
