...
Instrument::Instrument(const char *_name, u8 _type, u8 _volume)
  :NtMagic("INSN"), name(),type(_type), volume(_volume),samples(0), n_samples(0),
   note_samples(), 
...
{
  // name = (char*)_calloc(MAX_INST_NAME_LENGTH+1, 1);
	
	my_strncpy(name, _name, MAX_INST_NAME_LENGTH);
	
	// note_samples = (u8*)_calloc(sizeof(u8)*MAX_OCTAVE*12, 1);
...
Instrument::Instrument(const char *_name, Sample *_sample, u8 _volume)
  :NtMagic("INSN"), name(),type(INST_SAMPLE), volume(_volume),samples(0), n_samples(0),
   note_samples(), 
   n_vol_points(0), vol_env_on(true), vol_env_sustain(false), vol_env_loop(false),
   n_pan_points(0), pan_env_on(false), pan_env_sustain(false), pan_env_loop(false),
   envelope_ms(0), envelope_pixels(0)
{
  // name = (char*)_malloc(MAX_INST_NAME_LENGTH+1);
	for(u16 i=0; i<MAX_INST_NAME_LENGTH+1; ++i) name[i] = '\0';
	my_strncpy(name, _name, MAX_INST_NAME_LENGTH);
	
	samples = (Sample**)_malloc(sizeof(Sample*)*1);
	samples[0] = _sample;
	n_samples = 1;
	
	//note_samples = (u8*)_malloc(sizeof(u8)*MAX_OCTAVE*12);
  ...
  if (samples) {
    for(u8 i=0;i<n_samples;++i)
      delete samples[i];
		 ...
		 "smp@%p, notes@%p, name@%p", samples, note_samples, name);
  if(samples != NULL)
    _free(samples);
...
{
	n_samples++;
	samples = (Sample**)_realloc(samples, sizeof(Sample*)*n_samples);
...
void Instrument::setSample(u8 idx, Sample *sample)
{
	...
	if(n_samples < idx + 1)
	{
		samples = (Sample**)_realloc(samples, sizeof(Sample*) * (idx + 1));
